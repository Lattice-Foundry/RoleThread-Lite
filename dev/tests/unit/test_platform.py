...
def test_platform_support_messages_describe_windows_planned_features():
    ...
    assert "Installed app-window launch is handled through LitLaunch" in text
...
def test_platform_module_does_not_export_browser_or_launch_plan_residue():
    removed_names = (
        "BrowserInfo",
        "BrowserCapabilities",
        ...
        "LAUNCH_MODE_EDGE_WEBAPP",
        "LAUNCH_MODE_DEFAULT_BROWSER",
        "LAUNCH_MODE_MANUAL",
        "LAUNCH_MODE_UNSUPPORTED",
        "supports_edge_webapp",
        "supports_default_browser",
    )

    for name in removed_names:
        assert not hasattr(platform_helpers, name)

    windows_capabilities = platform_helpers.detect_platform("Windows").capabilities
    assert not hasattr(windows_capabilities, "supports_edge_webapp")
    assert not hasattr(windows_capabilities, "supports_default_browser")
...
def test_windows_platform_paths_use_localappdata_and_userprofile():
    ...
    assert paths.app_data_root == Path("C:/Users/Scott/AppData/Local/RoleThread")
    assert paths.workspace_root == Path("C:/Users/Scott/RoleThread")
    assert paths.training_data_dir == Path("C:/Users/Scott/RoleThread/training_data")
    assert paths.exports_dir == Path("C:/Users/Scott/RoleThread/exports")
    assert paths.imports_dir == Path("C:/Users/Scott/RoleThread/imports")
    assert paths.backups_dir == Path("C:/Users/Scott/RoleThread/backups")
    assert paths.logs_dir == paths.app_data_root / "logs"
    assert paths.cache_dir == paths.app_data_root / "cache"
    assert paths.database_path == paths.app_data_root / "rolethread.db"
...
def test_linux_platform_paths_use_xdg_style_app_data_and_home_workspace():
    ...
    assert paths.app_data_root == Path("/home/scott/.local/share/rolethread")
    assert paths.workspace_root == Path("/home/scott/RoleThread")
    assert paths.training_data_dir == Path("/home/scott/RoleThread/training_data")
    assert paths.exports_dir == Path("/home/scott/RoleThread/exports")
    assert paths.imports_dir == Path("/home/scott/RoleThread/imports")
    assert paths.backups_dir == Path("/home/scott/RoleThread/backups")
    assert paths.database_path == Path("/home/scott/.local/share/rolethread/rolethread.db")
...
def test_macos_platform_paths_use_application_support_and_home_workspace():
    ...
    assert paths.app_data_root == Path(
        "/Users/scott/Library/Application Support/RoleThread"
    )
    assert paths.workspace_root == Path("/Users/scott/RoleThread")
    assert paths.training_data_dir == Path("/Users/scott/RoleThread/training_data")
    assert paths.exports_dir == Path("/Users/scott/RoleThread/exports")
    assert paths.imports_dir == Path("/Users/scott/RoleThread/imports")
    assert paths.backups_dir == Path("/Users/scott/RoleThread/backups")
...
def test_unknown_platform_paths_use_safe_home_folder_fallback():
    ...
    assert paths.app_data_root == Path("/home/scott/RoleThread")
    assert paths.workspace_root == Path("/home/scott/RoleThread")
    assert paths.training_data_dir == Path("/home/scott/RoleThread/training_data")
    assert paths.backups_dir == Path("/home/scott/RoleThread/backups")
...
def test_default_onedrive_backup_path_appends_rolethread_folder(tmp_path, monkeypatch):
    one_drive = tmp_path / "OneDrive"
    one_drive.mkdir()
    monkeypatch.setattr(platform_helpers, "IS_WINDOWS", True)
    monkeypatch.setenv("ONEDRIVE", str(one_drive))
    ...
    assert platform_helpers.default_onedrive_backup_path() == (
        one_drive.resolve() / "RoleThread Lite" / "backups"
    )
